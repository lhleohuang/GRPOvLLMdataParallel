...
class GRPOTrainer(Trainer):
    ...
    def __init__(
        ...
        if self.use_vllm:
            ...
            if self.accelerator.is_main_process:
                self.vllm_clients = []
                vllm_server_ports = [8000, 8010]
                for server_port in vllm_server_ports:
                    self.vllm_clients.append(VLLMClient(
                        "0.0.0.0", server_port, connection_timeout=args.vllm_server_timeout
                    ))
                breakpoint()
                # self.vllm_client = VLLMClient(
                #         args.vllm_server_host, server_port, connection_timeout=args.vllm_server_timeout
                # )
                ...
                for name, param in self.model.named_parameters():
                    ...
                    if self.accelerator.is_main_process:
                        for vllm_client in self.vllm_clients:
                            # Update the vLLM client with the parameter data
    ...
    def generate_completions(args):
        client, prompts, num_generations, repetition_penalty, temperature, top_p, top_k, min_p, max_tokens, guided_decoding_regex = args
        ...
        if self.use_vllm:
            ...
            if self.accelerator.is_main_process:
                ...
                with profiling_context(self, "vLLM.generate"):
                    # Divide the generations into the number of vllm clients for data parallelism
                    num_clients = len(self.vllm_clients)
                    assert(self.args.per_device_train_batch_size == len(ordered_set_of_prompts))
                    num_generations_per_client = len(ordered_set_of_prompts) // num_clients # Divide the prompts instead of generations for each prompt
                    if len(ordered_set_of_prompts) % num_clients != 0:
                        raise ValueError(
                            f"Batch size ({self.args.per_device_train_batch_size}) must be evenly divisible by the "
                            f"number of vllm clients ({num_clients})."
                        )
                    # Generate completions in parallel using multiprocessing
                    import multiprocessing as mp
                    with mp.Pool(num_clients) as pool:
                        completion_ids = pool.map(
                            ...
                            [
                                ...
                                for client, prompts in zip(
                                    self.vllm_clients,
                                    [ordered_set_of_prompts[i : i + num_generations_per_client] for i in range(0, len(ordered_set_of_prompts), num_generations_per_client)],
                                )
                            ],
                        )
